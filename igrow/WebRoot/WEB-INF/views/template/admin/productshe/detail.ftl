...
<div class="titleH2 font14">${action.getText("product.label")}${action.getText("label.detail")}</div>
	  ...
	  <div class="OrderTable">
				    <form id="saveForm" method="post" action="${contextPath}/admin/productshe/<#if isAddAction>save<#else><#if result??>update<#else>save</#if></#if>.do" enctype="multipart/form-data">
                	...
                	<div class="products-vesting">
                    	<ul>
                        	<li style="display:none;"><label class="fixed-width">${action.getText("product.hotelName")}</label>
                        	<span><input type="text" class="hotelInput"  onclick="selectHotel()" readonly="readonly"  style="width:400px;"  value="${(result.hotelName)!}" id="hotelNameShow"/>
                        	<input type="hidden" id="hotelId" value="${(result.hotelId)!}" name="shelves.hotelId"/></span></li>
                            ...
                            <li><label class="fixed-width">${action.getText("product.name")}</label><span><input type="text" class="hotelInput" name="form.name" style="width:300px;" value="${(result.name)!}" datatype="*1-50" id="resultName"/></span><span id="resultNameSpan" class="resultNameSpan Validform_checktip"></span></li>
                            ...
                            <li><label class="fixed-width">佣金比例</label><span><input type="text" class="hotelInput shortInput" name="form.defSysCommRate" value="${(result.productInfo.defSysCommRate)!0}" id="PdefSysCommRate" maxlength="10" onkeyup="checkPlatPrice(this)" onafterpaste="this.value=this.value.replace(/[^0-9.]+/,'')"/>%</span><label class="tip_commission">*平台收取佣金</label></li>
                                     ...
                                     <#if productSpec.productConfigInfo?exists && productSpec.productConfigInfo??>
	                                     ...
	                                     <#assign configIndex = configIndex+1 >
	                                       <tr>
						                    ...
						                    <td width="125"><span class="empty15"><a class="file_update_area">
