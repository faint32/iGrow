<script type="text/javascript">
   ...
   $(function(){
      	...
      	$(".n_calculators input").bind("keyup", function(){
			...
			if(_val2 == null || _val2 == "" || _val2 == "平台佣金" ||  typeof(_val2) == "undefined"){_val2 =0 ;}
   ...
   function saveDefSysCommRate(id,obj,pid,type,sphotelConfId,productType){//老方法现在使用下面的方法进行操作
         if(confirm("确定要修改该商品的平台佣金比例吗？")){    
              ...
              var obj = {
                    ...
                    "form.sysCommRate":$(obj).prev().prev().val(),
   ...
   function saveHotelCommRate(id,obj,pid,type,sphotelConfId,productType){//老方法现在使用下面的方法进行操作
      ...
      function saveHotelSysCommRate(id,obj,pid,type,sphotelConfId,productType){//最新方法
		...
		var _valAfter2 = _val6.toFixed(10);
         ...
         if(confirm("确定要修改该商品的佣金比例吗？")){    
              var obj = {
                    ...
                    "shelves.sphotelConfId":sphotelConfId,
                    "shelves.sphotelConfType":type,
                    "form.hotelCommRate":_valAfter2,
                    "form.sysCommRate":_valAfter,
                    "form.productType":productType
              };
              $.ajax({
    			url : '${contextPath}/admin/productshe/saveHotelSysCommRate.do',
    			type : 'POST',
    			async : false,
    			dataType : "json",
    			async : false,
...
.products-vesting li span.n_calculators{ width:auto;}
</style>
    ...
    <div class="not_approved">
	  ...
	  <div class="OrderTable">
                	...
                	<div class="products-vesting ">
                    	<ul>
                        	<li><label class="fixed-width">${action.getText("product.name")}</label><span>${(result.name)!}</span>
                            ...
                            <li>
                            	<label class="fixed-width">平台佣金</label>
                            	<span id="sysdefSysCommRateyuan">
                             		<#if result!=null &&  result.productType != null && result.productType == 1>
	                            	<#if result.productInfo.sysCommRate != null>
	                            		${(result.productInfo.productPriceFirst*result.productInfo.sysCommRate/100)!"0"}<#else>${(result.productInfo.productPriceFirst*result.productInfo.defSysCommRate/100)!"0"}
	                            	</#if>
	                         		<#else>${(result.productInfo.productPriceFirst*result.productInfo.defSysCommRate/100)!"0"}</#if>元</span>
                        		<span id="sysdefSysCommRate" style="display:none;">
                             	<#if result!=null &&  result.productType != null && result.productType == 1>
	                            <#if result.productInfo.sysCommRate != null>
	                            	${(result.productInfo.sysCommRate)!0}<#else>${(result.productInfo.defSysCommRate)!0}
	                            </#if>
	                         	<#else>${(result.productInfo.defSysCommRate)!0}
	                         	</#if>
	                         	%
	                         	</span>
	                         	<input type="text" id="sysdefSysCommRateval" class="n_calculators_input" value="<#if result!=null &&  result.productType != null && result.productType == 1>
	                         	<#if result.productInfo.sysCommRate != null>
	                         	${(result.productInfo.sysCommRate)!0}<#else>${(result.productInfo.defSysCommRate)!0}</#if><#else>${(result.productInfo.defSysCommRate)!0}</#if>"  maxlength="10" onkeyup="checkPlatPrice(this)" onafterpaste="this.value=this.value.replace(/[^0-9.]+/,'')" style="display:none;"/>
	                         	<a href="#" onclick="editDefSysCommRate(this)" class="new_edit_font" style="display:none;">编辑</a><a href="#" onclick="saveDefSysCommRate('${(result.id)!}',this,'${(result.productInfo.id)!}',1,'${(result.sphotelConfId)!}','${(result.productType)!1}')" style="display:none; padding-left:5px;">保存</a>
	                         	<div class="ratioCalculation">
		                         	<span class="n_calculators" style="width:519px">
		                         		...
		                         		<table cellspacing="0" cellpadding="0" width="519">
		                         			<tr>
		                         				<td width="36%">
		                         				进价：${(result.productInfo.productPriceFirst)!0}
		                         		        <input type="hidden" placeholder="原价" readonly="readonly" value="${(result.productInfo.productPriceFirst)!0}" class="n_calculators_input" id="n_list_price" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'')" onafterpaste="this.value=this.value.replace(/[^0-9-]+/,'')"/>
		                         				</td>
		                         				<td width="37%">
		                         				<cite id="n_calculatorsValue"></cite>
		                         				</td>
		                         				<td width="27%">
		                         				</td>
		                         			</tr>
		                         			<tr>
		                         				<td>
		                         					平台：<input type="text" value=" <#if result!=null &&  result.productType != null && result.productType == 1><#if result.productInfo.sysCommRate != null>${(result.productInfo.productPriceFirst*result.productInfo.sysCommRate/100)!"0"}<#else>${(result.productInfo.productPriceFirst*result.productInfo.defSysCommRate/100)!"0"}</#if><#else>${(result.productInfo.productPriceFirst*result.productInfo.defSysCommRate/100)!"0"}</#if>" placeholder="平台佣金" class="n_calculators_input" id="n_system_price"/>
					                          	</td>
		                         				<td>
		                         					酒店：<input type="text" <#if result!=null &&  result.productType != null && result.productType == 0>readonly="readonly"</#if> placeholder="酒店佣金" value="<#if result!=null &&  result.productType != null && result.productType == 1><#if result.productInfo.hotelCommRate != null>${(result.productInfo.productPriceFirst*result.productInfo.hotelCommRate/100)!"0"}<#else>${(result.productInfo.productPriceFirst*result.productInfo.defHotelCommRate/100)!"0"}</#if><#else>0</#if>" class="n_calculators_input" id="n_hotel_price"/>
		                         				</td>
		                         				<td>
		                         				 	<a href="#" onclick="saveHotelSysCommRate('${(result.id)!}',this,'${(result.productInfo.id)!}',1,'${(result.sphotelConfId)!}','${(result.productType)!1}')" class="new_edit_font">保存</a>
		                         				</td>
		                         			</tr>
		                         		</table>
		                         	</span>
                            ...
                            <#if result!=null &&  result.productType != null && result.productType == 1>
                              <#if result.productInfo.hotelCommRate != null>
                                 <li><label class="fixed-width">酒店佣金</label><span id="sysdefHotelCommRateyuan">${(result.productInfo.productPriceFirst*result.productInfo.hotelCommRate/100)!"0"}元</span><span id="sysdefHotelCommRate" style="display:none;">${(result.productInfo.hotelCommRate)!0}%</span><input type="text" id="sysdefHotelCommRateval" class="n_calculators_input" value="${(result.productInfo.hotelCommRate)!0}"  maxlength="10" onkeyup="checkPlatPrice(this)" onafterpaste="this.value=this.value.replace(/[^0-9.]+/,'')" style="display:none;"/><a href="#" onclick="editDefSysCommRate(this)" class="new_edit_font" style="display:none;">编辑</a><a href="#" onclick="saveHotelCommRate('${(result.id)!}',this,'${(result.productInfo.id)!}',1,'${(result.sphotelConfId)!}','${(result.productType)!1}')" style="display:none; padding-left:5px;">保存</a></li>
                              <#else>
                                 <li><label class="fixed-width">酒店佣金</label><span id="sysdefHotelCommRateyuan">${(result.productInfo.productPriceFirst*result.productInfo.defHotelCommRate/100)!"0"}元</span><span id="sysdefHotelCommRate" style="display:none;">${(result.productInfo.defHotelCommRate)!0}%</span><input type="text" id="sysdefHotelCommRateval" class="n_calculators_input" value="${(result.productInfo.defHotelCommRate)!0}"  maxlength="10" onkeyup="checkPlatPrice(this)" onafterpaste="this.value=this.value.replace(/[^0-9.]+/,'')" style="display:none;"/><a href="#" onclick="editDefSysCommRate(this)" class="new_edit_font" style="display:none;">编辑</a><a href="#" onclick="saveHotelCommRate('${(result.id)!}',this,'${(result.productInfo.id)!}',2,'${(result.sphotelConfId)!}','${(result.productType)!1}')" style="display:none; padding-left:5px;">保存</a></li>
                              </#if>
                            </#if>
