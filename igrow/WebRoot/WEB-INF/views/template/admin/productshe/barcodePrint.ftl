...
<body>
<div id="p_main">
<div class="titleH2 font14"><span class="rightText barcordUpdate"><a class="printBarcode" onclick="printme()">打 印</a></span></div>
<!--<div id="print_show">
${(stockDto.hotel.name)!}&nbsp;&nbsp;<span class="nowTime"></span>
<p></p><br/>
<#if (pager.results)??>
    <#list pager.results as obj>
	<div class="barcord_area_n <#if obj_index%2==0>second_barcord_area_n</#if>">
		<div class="barcord_title_n">${(obj.productShelves.productInfo.name)!""}<br>￥${(obj.price)!}<br>(${(obj.specificationValue)!""}*${(obj.propertyValue)!""})</div>
	    <div class="barcord_img_n"><img src="http://www.1ct.cc:8080/borMall/barcode?msg=1<#if (obj.id)?length ==1>00000${(obj.id)}<#elseif (obj.id)?length ==2>0000${(obj.id)}<#elseif (obj.id)?length ==3>000${(obj.id)}<#elseif (obj.id)?length ==4>00${(obj.id)}<#elseif (obj.id)?length ==5>0${(obj.id)}<#else>${(obj.id)}</#if>9&type=code39" width="100%" /></div>
		<div class="clear"></div>
	</div>
    </#list>
</#if>
