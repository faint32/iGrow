<@a.header "${action.getText('ui.manage')}">
	<script>
		...
		$(function(){
		 ...
		 //查询
	     function list(){
	        $("#searchForm").attr("action","${contextPath}/admin/productshare/dayList.do");
	        $("#searchForm").submit();
	     }
	      //搜索
        ...
        <div class="contentBody">
           <!-- start content-left -->
            <div class="content-left">
                <div class="titleH2 font14">订单分享统计</div>
                    ...
                    <div class="OrderBottom">
                        <table width="558">
                            <tr>
                            	...
                            	<th width="110"><span class="empty15">分享数</th>
                                <th width="115"><span class="empty15">${action.getText("label.action")}</th>
                            </tr>
                            <#if (pager.results)??>
	                            <#list pager.results as obj>
		                            <tr <#if obj.orderDate == pager.fisrtId> class="change_bg_tr"</#if> onclick="dayDetails('${(obj.shareDate)!}');">
		                                <td><span class="empty15 ellipsis pro_name" title="${(obj.shareDate)!""}">${(obj.shareDate)!""}</span></td>
		                                <td><span class="empty15">${(obj.shareTotalQuantity)!'0'}</span></td>
		                                <td class="operating"><span class="empty15"><a class="zoomThis" onclick="dayDetails('${(obj.shareDate)!}');"></a></span></td>
		                            </tr>
	                            </#list>
	                        <#else>
	                        	    <tr>
