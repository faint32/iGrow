  ...
  function showDetail(id){
	  ...
	  if(serachId != null){
	       if(window.confirm("保存商品需要重新审核,是否继续操作?")){
	           var url = "${contextPath}/admin/product/detail.do?id="+id;
		       loadRightPage(url);
	      }
	   }
  }
...
<#if result != null && result.isAudit != null && result.isAudit.value != 0><a class="addProducts paddingL13" onclick="showDetail('${(result.id)!}')">编辑</a></#if>
	  ...
	  <div class="OrderTable">
				    <form id="saveForm" method="post" action="${contextPath}/admin/product/<#if isAddAction>save<#else><#if result??>savedetail<#else>save</#if></#if>.do" enctype="multipart/form-data">
                	...
                	<div class="products-vesting">
                    	<ul>
                        	<li><label class="fixed-width">${action.getText("product.name")}</label><span>${(result.name)!}<input type="hidden" class="hotelInput" name="form.name" value="${(result.name)!}" id="resultName" maxlength="50" /></span><span id="resultNameSpan" class="resultNameSpan Validform_checktip"></span></li>
                                     ...
                                     <#if productSpec.productConfigInfo?exists && productSpec.productConfigInfo??>
	                                     <#list productSpec.productConfigInfo as productConfigInfo>
	                                       ...
	                                       <tr>
										    ...
										    <td width="76"><span class="empty15"><input type="text" style="width:50px;" value="0" maxlength="8"  class="paddStock" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'')" onafterpaste="this.value=this.value.replace(/[^0-9-]+/,'')" onblur="addkucun(this)"/></td>
