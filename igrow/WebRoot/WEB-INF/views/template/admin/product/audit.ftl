<@a.header "${action.getText('ui.manage')}">
	<script>
		...
		$(function(){
		 ...
		 //查询
	     function list(){
	        $("#searchForm").attr("action","${contextPath}/admin/product/audit.do");
	        $("#searchForm").submit();
	     }
	       //搜索
	     ...
	     function edit(id,preId){
	        //设置每页第一条记录值
	     	$("#fisrtId").val(id);
	     	var url = "${contextPath}/admin/product/auditDetail.do?id="+id+"&preId="+preId;
	        loadRightPage(url);
	     }
	     //加载页面
	        		...
	        		setTimeout(function(){
						tip_hide_w(); 
							if($("#pagerTotalCount").html() == 0 ){
					            $(".content-right .OrderTable").html("<div class='noData'><p>暂无待审核商品</p></div>");
					        }
					},300);
	        	}
			}); 
...
<body>
                ...
                <div class="OrderTable">
                    <div class="OrderTop">
                        <div class="searchOrderDiv">
                         <form action="${contextPath}/admin/product/audit.do" method="post" id="searchForm">
                    ...
                    <div class="OrderBottom">
                        <table width="100%">
                            <tr>
                            	...
                            	<th width="15%"><span class="empty15">进价</span></th>
                            ...
                            <#if (pager.results)??>
	                            <#list pager.results as obj>
		                            ...
		                            <tr <#if obj.id == pager.fisrtId> class="change_bg_tr"</#if>>
		                                <td onclick="edit(${(obj.id)!},${(preId)!});"><span class="empty15">${(obj.productSn)!""}</span></td>
		                                <td onclick="edit(${(obj.id)!},${(preId)!});"><span class="empty15 order_Number ellipsis" title="${(obj.name)!""}">${(obj.name)!""}</span></td>
		                                <td onclick="edit(${(obj.id)!},${(preId)!});"><span class="empty15">${(obj.productPrice)!0}</span></td>
		                                <td onclick="edit(${(obj.id)!},${(preId)!});"><span class="empty15 pro_name ellipsis" title="<#if obj.productType == 0>${(obj.hotelName)!""}<#else>${(obj.supplierInfo.name)!""}</#if>"><#if obj.productType == 0>${(obj.hotelName)!""}<#else>${(obj.supplierInfo.name)!""}</#if></span></td>
		                                <td onclick="edit(${(obj.id)!},${(preId)!});"><span class="empty15">${(obj.isAudit.desc)!}</span></td>
 ...
 <div class="layer" id="closeAudit">
    ...
    <div class="layer-content tip_why_this write_why">
    	<div class="why_tip">
            <b class="layer_title"><span>未通过原因</span><a class="btn_closedPage" onclick="hidecloseAudit()"></a></b>
            <div class="write_whyNo">
