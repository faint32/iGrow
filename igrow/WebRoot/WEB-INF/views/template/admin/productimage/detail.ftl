...
<script type="text/javascript">
  ...
  function save(){
	      $(document.getElementsByTagName("iframe")[0].contentWindow.document.body).find("img").each(function(){
            ...
            $(document.getElementsByTagName("iframe")[1].contentWindow.document.body).find("img").each(function(){
                 if($.trim($(this).attr("data-cke-saved-src").substring(0,10)) == "data:image"){
                       $(this).attr("data-cke-saved-src","");
                 }else if($.trim($(this).attr("src").substring(0,10)) == "data:image"){
                       $(this).attr("src","");
 ...
 function saveOther(){
          ...
          $(document.getElementsByTagName("iframe")[1].contentWindow.document.body).find("img").css({"width":"100%", "height":"auto"})
          $(document.getElementsByTagName("iframe")[1].contentWindow.document.body).find("img").each(function(){
                 if($.trim($(this).attr("data-cke-saved-src").substring(0,10)) == "data:image"){
                       $(this).attr("data-cke-saved-src","");
                 }else if($.trim($(this).attr("src").substring(0,10)) == "data:image"){
           ...
           var content3=$(document.getElementsByTagName("iframe")[1].contentWindow.document.body).html();
           $("#editor2").val(content3);
...
<div class="titleH2 font14"><#if isAddAction>${action.getText("button.addnew")}<#else><#if result??>${action.getText("button.edit")}<#else>${action.getText("button.addnew")}</#if></#if>${action.getText("product.label")}${action.getText("label.detail")}
	  ...
	  <div class="OrderTable">
                	...
                	<div class="products-vesting">
                    	<ul>
                        	<li class="lineHeight34"><label class="fixed-width">${action.getText("product.name")}</label>
                        		<span><input type="text" class="hotelInput" name="form.name" value="${(result.name)!}" id="resultName"  datatype="*1-50" /></span>
                            ...
                            <li><label class="fixed-width">${action.getText("product.deliveryModes")}</label><span>
                                ...
                                <input type="hidden" name="form.deliveryModes" id="deliveryModesStr" style="display:none;"/>
                            </li>
                            
                            <li class="lineHeight34"><label class="fixed-width">默认销量</label>
                        		<span><input type="text" class="hotelInput" name="form.defSalesCount" value="${(result.defSalesCount)!0}" maxlength=10  onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')"/></span>
                        	</li>
                            
                            <li style="display:none;"><label class="fixed-width">平台佣金</label><span><input type="text" class="hotelInput shortInput" name="form.defSysCommRate" value="${(result.defSysCommRate)!0}" id="PdefSysCommRate" maxlength="10" onkeyup="checkPlatPrice(this)" onafterpaste="this.value=this.value.replace(/[^0-9.]+/,'')"/><i class="">%</i></span><label class="tip_commission">*平台收取佣金</label></li>
                            <li style="display:none;"><label class="fixed-width">酒店佣金</label><span><input type="text" class="hotelInput shortInput" name="form.defHotelCommRate" value="${(result.defHotelCommRate)!0}" id="PdefHotelCommRate" maxlength="10" onkeyup="checkPlatPrice(this)" onafterpaste="this.value=this.value.replace(/[^0-9.]+/,'')"/><i class="">%</i></span><label class="tip_commissionblack">可选填</label></li>
                            ...
                            <a class="file_update_area" onclick="loadimage('productImg','imageSrcshow')">
                             ...
                             <li id="imageSrcDiv">
                             	<label class="fixed-width"></label><span class="reviewPic"><img id="imageSrcshow" src="${imagePath}${(result.imageUrl)!}"></span>
                             </li>
                             <#else> 
                    ...
                    <div class="OrderBottom">
                        <table width="760" class="blockTableBottom specMameTable" cellpadding="0" cellspacing="0">
                            <tr>
								...
								<th width="217" <#if result?? && result !=null>style="display:none;"</#if>>
                            ...
                            <#if result??&&result.productSpec?exists && result.productSpec??>
                                <#list result.productSpec as productSpec>
                                 <tr>
	                                ...
	                                <td width="290"><span class="empty15"><span class="text-put-here"   name="${(productSpec.id)!}">${(productSpec.value)!}</span><span style="display:none;"><input type="text" onblur="saveProductProperty(this,2)" maxlength="20"/></span></span></td>
	                                <td width="217" <#if result?? && result !=null>style="display:none;"</#if>><span class="empty15"><a class="toUp" onclick="move_up(this)">上移</a><a class="deleteThisCole" name="1">删除</a></span></td>
                                  </tr>
                               </#list>
                            </#if>
                            <tr <#if result?? && result !=null>style="display:none;"</#if>>
                                ...
                                <td width="217" class="show-two-btn"><span class="empty15"><a class="addTd" name="1" onclick="add_tr(this)">添加</a><a class="toUp" onclick="move_up(this)">上移</a></span></td>
                            </tr>
                        </table>
                            ...
                            <#if result??&&result.productProperty?exists && result.productProperty??>
                                <#list result.productProperty as productProperty>
                                 <tr>
	                                ...
	                                <td width="217" <#if result?? && result !=null>style="display:none;"</#if>><span class="empty15"><a class="toUp" onclick="move_up(this)">上移</a><a class="deleteThisCole" name="2">删除</a></span></td>
                                  </tr>
                               </#list>
                            </#if>
                            <tr <#if result?? && result !=null>style="display:none;"</#if>>
                                     ...
                                     <#if productSpec.productConfigInfo?exists && productSpec.productConfigInfo??>
	                                     <#list productSpec.productConfigInfo as productConfigInfo>
	                                       ...
	                                       <tr>
										    ...
										    <td width="76" style="display:none;"><span class="empty15">${(productConfigInfo.totalStock-productConfigInfo.lockedStock)!0}<input type="hidden" style="width:50px;" value="${(productConfigInfo.totalStock-productConfigInfo.lockedStock)!}" maxlength="8" class="ptotalStock" <#if result.id != null>readonly="readonly" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" </#if>/></span></td> 
										    <td width="76" style="display:none;"><span class="empty15"><input type="text" style="width:50px;" value="" maxlength="8"  class="paddStock" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'')" onafterpaste="this.value=this.value.replace(/[^0-9-]+/,'')" onblur="addkucun(this)"/></td>
										    ...
										    <!--<td width="125"><span class="empty15">
										    <a class="file_update_area">
                        	...
                        	<div class="pro_title_top">
							    ...
							    <div class="content-table contenter_summary blockActiveList" id="tagContent">
							        ...
							        <div id="tagContent1" class="tagContent">
							        	<textarea cols="80" id="editor2" name="form.experienceAfter"  rows="10">
