<@a.header "${title}">
	<script>
		...
		$(function(){
		 ...
		 //列表
	     function list(){
	        $("#searchForm").attr("action","${contextPath}/admin/hotel/list.do");
	        $("#searchForm").submit();
	     }
	     //搜索
	    ...
	    //添加
	     function add(){
	     	...
	     	//加载添加页面
	     	var url = "${contextPath}/admin/hotel/add.do";
	        loadRightPage(url);
	     }
	     //编辑
	     function edit(id){
	     	//设置每页第一条记录值
	     	$("#fisrtId").val(id);
	     	var url = "${contextPath}/admin/hotel/edit.do?id="+id;
	        loadRightPage(url);
	     }
	     //加载页面
	     ...
	     function del(id,preId,name){
	      if(confirm("该酒店的商品数据也将被删除，是否继续操作？")){
		     $.ajax({    
		        type:'POST',        
		        url:'${contextPath}/admin/hotel/delete.do?id='+id,    
		        data:null,    
		        cache:false,    
		        dataType:'json',    
        ...
        <div class="contentBody">
           <!-- start content-left -->
            <div class="content-left">
                <div class="titleH2 font14">${action.getText("hotel.label")}${action.getText("label.list")}<a class="addProducts" onclick="add();" ><img src="${contextPath}/css/images/white_add_icon.png">${action.getText("button.addnew")}${action.getText("hotel.label")}</a></div>
                    ...
                    <div class="OrderBottom">
                        <table width="100%">
                            <tr>
                                ...
                                <th width="53%"><span class="empty15">${action.getText("hotel.label.name")}</span></th>
                                <th width="14%"><span class="empty15">酒店编号</span></th>
                            ...
                            <#if (pager.results)??>
	                            <#list pager.results as obj>
		                            ...
		                            <tr<#if obj.id == pager.fisrtId> class="change_bg_tr"</#if> onclick="edit(${(obj.id)!});">
		                                ...
		                                <td><span class="empty15">${obj.code}</span></td>
		                                <td class="operating"><span class="empty15">
		                                	<a class="two-di" <#if obj!=null>onclick="showErweima('${obj.name}','${imagePath}${(obj.getRealHotelUrl())!}')"</#if>></a>
		                                	<a class="editThis" onclick="edit(${(obj.id)!});"></a>
		                                	<a class="deleteThis" onclick="del('${(obj.id)!}','${(preId)!}','${(obj.name)!}');"></a>
		                                 </span>
		                                </td>
		                            </tr>
		                            <#assign preId=obj.id/>
