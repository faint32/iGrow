  ...
  function showDetail(id){
	  ...
	  if(serachId != null){
	       if(window.confirm("保存商品需要重新审核,是否继续操作?")){
	           var url = "${contextPath}/admin/productsheimage/detail.do?id="+id;
		       loadRightPage(url);
	      }
	   }
  }
...
<#if result != null && result.productInfo != null && result.productInfo.isAudit != null && result.productInfo.isAudit.value != 0><a class="addProducts paddingL13" onclick="showDetail('${(result.id)!}')">编辑</a></#if> 
	  ...
	  <div class="OrderTable">
				    <form id="saveForm" method="post" action="${contextPath}/admin/productsheimage/<#if isAddAction>save<#else><#if result??>savedetail<#else>save</#if></#if>.do" enctype="multipart/form-data">
					...
					<input type="hidden" name="shelves.status" value="${(result.status)!}" />
                                     ...
                                     <#if productSpec.productConfigInfo?exists && productSpec.productConfigInfo??>
	                                     ...
	                                     <#assign configIndex = configIndex+1 >
	                                       <tr>
										    ...
										    <td width="76"><span class="empty15">
										    ...
										    <input type="hidden" style="width:50px;" value="${(productConfigInfo.productPrice)!}" maxlength="8" class="pproductPrice"  onkeyup="clearNoNum(this)"/></span></td> 
										    ...
										    <td width="76" style="display:none;"><span class="empty15"><input type="text" style="width:50px;" value="0" maxlength="8"  class="paddStock" onkeyup="this.value=this.value.replace(/[^0-9-]+/,'')" onafterpaste="this.value=this.value.replace(/[^0-9-]+/,'')" onblur="addkucun(this)"/></span></td> 
